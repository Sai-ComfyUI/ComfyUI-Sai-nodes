...
# Vendored from https://github.com/vaskers5/LUA (with ComfyUI compatibility adjustments).
...
class SwinIRMultiHead(nn.Module):
    ...
    def __init__(
        ...
        # Keep these scalar schedule values on CPU so callers can construct the
        # weight-heavy module on the meta device before assigning a checkpoint.
        dpr = torch.linspace(
            0, drop_path_rate, sum(depths), device="cpu"
        ).tolist()
